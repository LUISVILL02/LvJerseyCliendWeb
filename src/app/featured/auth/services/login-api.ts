...
export class LoginApi {
  ...
  loginWithCredentials(userName: string, password: string) {
    ...
    return this.httpClient.post<{ accessToken: string, refreshToken: string }>(`${this.url}/Auth/login`, { userName, password })
      .pipe(
        takeUntilDestroyed(this.destroyRef),
        catchError((error: HttpErrorResponse) => {
          this.loading.set(false);
          this.errorResponse.set({ error: true, message: error.error.message || 'Error del servidor' });
      ...
      .subscribe(response => {
        ...
        if (isPlatformBrowser(this.platformId)) {
          localStorage.setItem('tokenAccessLvJersey', response.accessToken);
          localStorage.setItem('refreshTokenAccessLvJersey', response.refreshToken);
        }
  ...
  logout = () => {
    this.userAuth.setTokenSignal('');
    this.userAuth.setRefreshTokeSignal('');
